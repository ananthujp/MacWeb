...
import { TabButton, Text, Window } from "../Colors/Window";
  ...
  return (
    <div className="flex flex-row w-full h-full">
      <motion.div
        ...
        className={
          " flex-col  backdrop-blur-xl h-full w-48 max-w-48 " +
          (sidebar ? " flex" : " hidden ") +
          Window.Glass
        }
      >
        ...
        <div className="px-2 overflow-y-auto cursor-default">
          {sideBarItems.map((item) => (
            <div
              className={
                "flex px-2 flex-col my-2 text-xs font-semibold " +
                Text.lightShade
              }
            >
              ...
              {item.items.map((dc) => (
                <div className="flex hover:bg-gray-200/30 active:bg-gray-300/30 group mt-2 flex-row justify-between text-md py-1 font-normal pl-2 rounded-md">
                  <div className="flex flex-row">
                    <div className="text-blue-500">{dc.icon}</div>
      ...
      <div
        className={
          "windowHandle flex flex-col w-full h-full " + Window.TitleBar
        }
      >
        <div
          className={
            "px-4 cursor-default flex flex-row justify-between items-center pl-4 text-sm font-bold text-gray-600 h-12 border-b  " +
            Window.TitleBar +
            TabButton[2]
          }
        >
